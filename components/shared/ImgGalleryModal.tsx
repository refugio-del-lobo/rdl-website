...
const ImgGalleryModal: React.FC<ComponentProps> = ({toggleModalProp,imgGroupLinksProp,selectedImgProp}) => {
  ...
  return (
    <div className="fixed top-0 left-0 w-screen h-screen bg-[#000000f6] flex justify-center items-center z-50">
        ...
        <div className="relative wrapper flex flex-col w-full h-full items-center">
          <Image src={"/assets/icons/exit-icon.svg"} width={28} height={28} alt="exit icon" className="absolute top-0 right-0 m-5 md:m-8 z-50 w-8 h-8 md:w-10 md:h-10 cursor-pointer" onClick={toggleModalProp}/>
          ...
          {/* ******************   Carrusel de imágenes para seleccionar ************ */}
            <div className="w-auto max-w-full grid grid-flow-col auto-cols-max gap-2 overflow-x-auto overscroll-x-contain p-2
            snap-x 
